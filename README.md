...
Each fixed issue on a package carries **only what a tracker told us**: the Azure Boards work item the testers file, the HaloITSM ticket named in that work item's `SM Problem` field, the work item's own title and state, and — when it is known — the office that reported it. Everything but the work item id is read by the lookup rather than typed. The title and the state are stored with the issue so reopening a package shows what was found instead of re-querying the tracker for every row; they are a snapshot of the day the issue was added, and ⟳ on a row asks for the current values. What the release actually changes is described **once for the whole package**, in its own section, rather than a line per issue. Both ids are shown to the deployer during the rollout, linked through the project's tracker settings (or `ISSUE_TRACKER_URL` / `WORKITEM_URL` as the instance-wide fallback).
