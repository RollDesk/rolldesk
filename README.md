...
Each fixed issue on a package carries **only what a tracker told us**: the Azure Boards work item the testers file, the service-desk ticket named in that work item's `SM Problem` field, the work item's own title and state, and — when it is known — the office that reported it. Everything but the work item id is read by the lookup rather than typed. The title and the state are stored with the issue so reopening a package shows what was found instead of re-querying the tracker for every row; they are a snapshot of the day the issue was added, and ⟳ on a row asks for the current values. The office has no column in the editor — nobody reads it there — but it is stored, because it drives the rollout order (below). What the release actually changes is described **once for the whole package**, in its own section, rather than a line per issue; **Move to the release description** writes a line per listed issue into that text, appending and skipping what is already there. Both ids are shown to the deployer during the rollout, linked through the project's tracker settings (or `ISSUE_TRACKER_URL` / `WORKITEM_URL` as the instance-wide fallback).

An application on a package may be **released as a set of services**, each with its own version. Some applications are not one artefact — a portal can be nineteen containers cut from one release, seventeen of them on the same version and two behind — and neither one row with one version nor nineteen separate applications could say that: the application name is the identity every downstream view is keyed by (the install report, the per-application progress, the client's changelog, the worklist). So the services hang off the application entry, the release stays one row, and **a service inherits the application's version unless it names one of its own** — only the difference is stored, which is why correcting the release version still reaches every service that was not deliberately pinned away from it.

The service names are declared once on the application in its project (one per line; a `:tag` is dropped, because a version belongs to a release rather than to a catalogue), and a package then **ticks the ones that are going out** — nothing is selected by default, since a release is as often one service as all of them, and filling in every service is never required. A service the catalogue does not know can be added inside the package, so a new service does not block a tester (who cannot edit projects), and `📋 Wklej listę` takes the image list a pipeline prints (`pudo/auth-service:2.7.0-dev.27504`, one per line): it ticks what it matches, adds what it does not, adopts the version most of the list shares as the release version, and leaves only the ones that differ on their rows. The list is read as a `🧩 Serwisy` panel in the deployer panel and the deployments list, as a count wherever an application is listed, as a collapsed block in the client's portal, and as a count plus the exceptions in a notification. Changing which services a release carries, or pinning one, **clears the project manager's approval** exactly as changing a version does.

Each application version on a package may also say **where its build is** — a URL or a UNC share. The address travels with the version to the deployments list and the deployer panel, including a chip on the deployer's action bar, so nobody has to ask for the path of the file over chat. A web address is rendered as a link; a share is offered as a path to copy, since a browser cannot navigate to one. It is read live from the package (a corrected path reaches rollouts already planned) and counts as deployer material: `stripAdminInfoFromPackage` removes it for a client account on a project that does not share deployment information, alongside the instructions.
...
These settings also decide **where a reader's browser goes** when they click an id. The work item link is derived from the organisation and project above — Azure DevOps serves a work item at a fixed route, so there is nothing to type and nothing that can disagree with the lookup. The ticket link is the setting in the last row, because a service desk's web view cannot be derived from its API host. A project's own patterns take precedence over the environment-wide `ISSUE_TRACKER_URL` / `WORKITEM_URL`, which remain the fallback for projects that configure no tracker of their own — a single instance serves clients in different organisations, so one pattern cannot be right for all of them. Client accounts are never sent either pattern: neither of our trackers is reachable from outside, so a client reads the ids as text.
