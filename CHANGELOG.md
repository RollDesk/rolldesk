...
- **The schedule can be sorted** by deployment date (newest or oldest first), by project, or by when it was added — the deployments list was fixed at most-recently-added, which made finding a particular rollout in a long list slow. Records without a date sort last in both directions.
